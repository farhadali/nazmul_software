...
(function($, window, document, undefined) {
	...
	AmsifySuggestags = function(selector) {
		...
		this.classes       = {
			...
			tagInputGroup : '.amsify-input-group',
			listArea      : '.amsify-suggestags-list',
			list          : '.amsify-list',
			listItem      : '.amsify-list-item',
			itemPad       : '.amsify-item-pad',
			inputType     : '.amsify-select-input',
			tagArea       : '.amsify-select-tag-area',
			tagItem       : '.amsify-select-tag',
			plusItem 	  : '.amsify-input-group-addon', //amsify-plus-tag
			colBg         : '.col-bg',
	...
	AmsifySuggestags.prototype = {
		...
		*/
		...
		createHTML : function() {
			...
			var tagInputGroupHtml     = '<div class="'+this.classes.tagInputGroup.substring(1)+'"></div>';
			this.selectors.tagInputGroup  = $(tagInputGroupHtml).appendTo(this.selectors.inputArea);
            
			var sTagsInput            = '<input type="text" class="'+this.classes.sTagsInput.substring(1)+'" placeholder="'+this.defaultLabel+'">';
			this.selectors.sTagsInput = $(sTagsInput).appendTo(this.selectors.tagInputGroup).attr('autocomplete', 'off');
            
			var tagAreaHTML           = '<div tabindex="0" class="'+this.classes.tagArea.substring(1)+'"></div>';
			this.selectors.tagArea  = $(tagAreaHTML).appendTo(this.selectors.sTagsArea);
		...
		setRemoveEvent: function() {
			var _self = this;
			$(this.selectors.sTagsArea).find(this.classes.removeTag).click(function(e){
		...
		addTag : function(value, animate=true) {
            ...
            if(this.settings.showPlusAfter){
                $item    = $(html).appendTo($(this.selectors.tagArea));
            } else {
			...
			if(this.settings.showPlusAfter) {
				if(this.tagNames.length > 0) {
					...
					if(!this.selectors.plusTag) {
						...
						this.selectors.plusTag 	= $(html).insertAfter(this.selectors.sTagsInput);
					...
					$(this.selectors.plusTag).html(plusNumber + ' <b class="icon-arrow-down"></b>');
                    
					var $tagItems = $(this.selectors.tagArea);
                    $tagItems.width($(this.selectors.sTagsArea).width()+5);
                    
                    $(this.selectors.plusTag).click(function(){
                        $tagItems.show();
                        $tagItems.focus();
                    });
                    $tagItems.blur(function(){
                        $tagItems.hide();
                    });
					...
					//$(this.selectors.plusTag).hide();
                    $(this.selectors.plusTag).html('0 <b class="icon-arrow-down"></b>');
                    $(this.selectors.tagArea).hide();
				}
			}
		}
	};
